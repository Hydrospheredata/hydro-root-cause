...
SERVICENAME = 'hydro-root-cause'
SEARCHPATH = './requirements.txt'
SEARCHSDK = 'hydrosdk'
SEARCHGRPC = 'hydro-serving-grpc'
REGISTRYURL = 'hydrosphere'
GITHUBREPO  = "github.com/Hydrospheredata/hydro-root-cause.git"
...
def updateHelmChart(String newVersion){
  dir('helm'){
    //Change template
    sh script: "sed -i \"s/.*full:.*/  full: hydrosphere\\/${SERVICENAME}:$newVersion/g\" rootcause/values.yaml", label: "sed ${SERVICENAME} version"
    sh script: "sed -i \"s/.*hydrosphere\\/${SERVICENAME}.*/    full: hydrosphere\\/${SERVICENAME}:$newVersion/g\" dev.yaml", label: "sed ${SERVICENAME} dev stage version"

    //Refresh readme for chart
    sh script: "frigate gen rootcause --no-credits > rootcause/README.md"

    dir('rootcause'){
        sh script: "helm lint .", label: "Lint stat chart"
